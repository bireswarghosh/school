...
import { Pencil, Trash2, X, Save, Check, Filter, LogIn, Ban, CheckCircle, Loader2 } from "lucide-react"
...
export default function StudentsPanel() {
  ...
  const { data: students, loading, update, remove } = useApi<StudentUser>("/api/system-setting/student-users")
  ...
  const filtered = students.filter((s) => {
    if (filterStatus !== "All" && String(s.status).toLowerCase() !== filterStatus.toLowerCase()) return false
    if (search) {
  ...
  const handleEditOpen = (s: StudentUser) => {
    setEditing(s)
    setForm({ name: s.name, email: s.email || "", status: String(s.status).toLowerCase() === "active" })
    setShowModal(true)
  }
  ...
  const confirmDelete = async () => {
    ...
    setShowDelete(false)
    setDeleteId(null)
    showSuccess("Student user deleted successfully!")
  }
  ...
  const handleBulkAction = async () => {
    ...
    showSuccess(`${ids.length} student(s) ${newStatus === "active" ? "enabled" : "disabled"} successfully!`)
  }
    ...
    <>
      ...
      <div className="flex items-center justify-between gap-4">
        <div className="flex items-center gap-3">
          ...
          <input
            type="text"
            placeholder="Search by name, username, email, admission no..."
            value={search}
        ...
        <span className="text-sm text-gray-500">{filtered.length} student(s)</span>
      </div>
          ...
          <table className="w-full text-sm">
            <thead>
              <tr className="border-b border-gray-200 bg-gray-50">
                ...
                {["Photo", "Admission No", "Name", "Username", "Class", "Section", "Gender", "Email", "Status", "Last Login", "Action"].map((h) => (
            ...
            <tbody>
              {loading && filtered.length === 0 ? (
                ...
                <tr><td colSpan={12} className="text-center py-8 text-gray-400">No students found</td></tr>
              ) : (
                filtered.map((s, idx) => {
                  ...
                  return (
                    <tr key={s.id} className={`border-b border-gray-100 hover:bg-gray-50 transition-colors ${idx % 2 === 0 ? "bg-white" : "bg-gray-50/50"}`}>
                      ...
                      <td className="px-4 py-3">
                        {s.studentPhoto ? (
                          <img src={s.studentPhoto} alt="" className="h-8 w-8 rounded-full object-cover" />
                        ) : (
                      ...
                      <td className="px-4 py-3 font-medium text-gray-800">{s.admissionNo || "-"}</td>
                      ...
                      <td className="px-4 py-3">
                        <button
                          onClick={() => setConfirmAction({ type: isActive ? "disable" : "enable", userId: s.id })}
                      ...
                      <td className="px-4 py-3 text-gray-500 text-xs whitespace-nowrap">{s.lastLogin || "Never"}</td>
                      <td className="px-4 py-3">
                        <div className="flex items-center gap-1">
                          ...
                          <button onClick={() => { setDeleteId(s.id); setShowDelete(true) }} className="p-1.5 text-red-600 hover:bg-red-50 rounded-lg transition-colors" title="Delete">
                            <Trash2 className="h-4 w-4" />
                          </button>
                          {canImpersonate && (
                            <button onClick={() => handleAutoLogin(s.id)} disabled={loginAsId === s.id} className="p-1.5 text-green-600 hover:bg-green-50 rounded-lg transition-colors disabled:opacity-50" title="Login as this student">
                              {loginAsId === s.id ? <Loader2 className="h-4 w-4 animate-spin" /> : <LogIn className="h-4 w-4" />}
                            </button>
                          )}
                        </div>
                      </td>
        ...
        <div className="fixed inset-0 z-50 flex items-center justify-center bg-black/50">
          <div className="w-full max-w-md rounded-xl bg-white p-6 shadow-lg">
            <div className="flex items-center justify-between mb-4">
              <h3 className="text-base font-semibold text-gray-800">Edit Student</h3>
        ...
        <div className="fixed inset-0 z-50 flex items-center justify-center bg-black/50">
          <div className="w-full max-w-md rounded-xl bg-white p-6 shadow-lg">
            ...
            <p className="text-sm text-gray-600">
              Are you sure you want to delete this student user account?
              {deleteId && <strong className="block mt-1 text-gray-800">{students.find((s) => s.id === deleteId)?.name}</strong>}
            </p>
            <div className="mt-6 flex justify-end gap-2">
          ...
          <div className="w-full max-w-md rounded-xl bg-white p-6 shadow-lg">
            <div className="flex items-center justify-between mb-4">
              <h3 className="text-base font-semibold text-gray-800">
                {confirmAction.type.includes("disable") ? "Disable Student" : "Enable Student"}
              </h3>
            ...
            <p className="text-sm text-gray-600">
              {confirmAction.type.includes("bulk")
                ? `Are you sure you want to ${confirmAction.type.includes("disable") ? "disable" : "enable"} ${selectedIds.length} selected student(s)?`
                : `Are you sure you want to ${confirmAction.type === "disable" ? "disable" : "enable"} this student?`
              }
              {confirmAction.type === "disable" && (
                <span className="block mt-2 text-xs text-amber-600">Disabled students cannot log in until re-enabled.</span>
              )}
            </p>
            <div className="mt-6 flex justify-end gap-2">
