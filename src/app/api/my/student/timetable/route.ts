...
export const GET = handle(async (req: NextRequest, ctx) => {
  ...
  const res = await query(
    `SELECT te.id, te.subject_name AS "subject", te.day, te.period, te.start_time AS "startTime",
       te.end_time AS "endTime", te.teacher_name AS "teacher"
  ...
  return { studentId: Number(student.id), timetable: res.rows }
})
