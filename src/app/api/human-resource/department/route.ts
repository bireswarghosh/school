...
export async function POST(req: NextRequest) {
  try {
    ...
    // Bulk upload: { names: string[] } or { bulk: string } — each line is Department Name
    ...
    if (bulkNames) {
      // normalize: trim, filter empty, dedup case-insensitive (preserve first casing), max 500 lines
      ...
      // fetch existing names for this school to skip duplicates
      ...
      try {
        ...
        // if table empty or query fails, treat as no existing
      }
      ...
      return NextResponse.json(
        { created, skipped, errors, total: bulkNames.length, cleaned: cleaned.length },
        { status: 201 }
      )
    }
