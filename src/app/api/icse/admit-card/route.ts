...
export async function POST(req: NextRequest) {
  try {
    ...
    const result = await query(
      `INSERT INTO icse_admit_cards (student_id, student_name, admission_no, class, section, exam_name, roll_no, father_name, mother_name, date_of_birth, photo)
...
export async function PUT(req: NextRequest) {
  try {
    ...
    vals.push(id)
    const result = await query(`UPDATE icse_admit_cards SET ${sets.join(", ")} WHERE id = $${idx} RETURNING *`, vals)
...
export async function DELETE(req: NextRequest) {
  ...
  if (!id) return NextResponse.json({ error: "id required" }, { status: 400 })
  await query("DELETE FROM icse_admit_cards WHERE id = $1", [id])
