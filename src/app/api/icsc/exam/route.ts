...
export async function GET(req: NextRequest) {
  ...
  if (id) {
    ...
    const rows = await query(`SELECT COUNT(*)::int AS count FROM icsc_exam_subjects WHERE exam_id = $1`, [parseInt(id)])
  ...
  const enriched = await Promise.all(items.map(async (item: any) => {
    const rows = await query(`SELECT COUNT(*)::int AS count FROM icsc_exam_subjects WHERE exam_id = $1`, [item.id])
