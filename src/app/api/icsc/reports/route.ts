  ...
  try {
    ...
    if (type === "template-marks") {
      const result = await query(`
        ...
        FROM icsc_templates ORDER BY name
      `)
    ...
    const result = await query(`
      ...
      FROM icsc_exam_subjects sub
      JOIN icsc_exams e ON e.id = sub.exam_id
      LEFT JOIN icsc_exam_students es ON es.exam_id = e.id
      LEFT JOIN icsc_exam_marks em ON em.subject_id = sub.id AND em.student_id = es.student_id
