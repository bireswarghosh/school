...
export async function DELETE(req: NextRequest) {
  ...
  if (examId && subjectId) {
    await query("DELETE FROM cbse_exam_marks WHERE exam_id = $1 AND subject_id = $2", [parseInt(examId), parseInt(subjectId)])
    return NextResponse.json({ success: true })
  }
  if (examId) {
    await query("DELETE FROM cbse_exam_marks WHERE exam_id = $1", [parseInt(examId)])
    return NextResponse.json({ success: true })
  }
