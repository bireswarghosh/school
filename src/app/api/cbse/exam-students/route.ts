...
const TABLE = "cbse_exam_students"
const ORDER = "id ASC"
...
export async function DELETE(req: NextRequest) {
  ...
  if (examId && studentId) {
    await query("DELETE FROM cbse_exam_students WHERE exam_id = $1 AND student_id = $2", [parseInt(examId), parseInt(studentId)])
    return NextResponse.json({ success: true })
  }

  if (examId) {
    await query("DELETE FROM cbse_exam_students WHERE exam_id = $1", [parseInt(examId)])
    return NextResponse.json({ success: true })
  }
