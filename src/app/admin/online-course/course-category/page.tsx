...
export default function CourseCategoryPage() {
  const { data: categories, add, update, remove } = useApi<Category>("/api/online-course/course-category")
      ...
      <div className="relative overflow-hidden rounded-2xl bg-gradient-to-br from-violet-600 via-indigo-600 to-purple-700 px-6 py-6 shadow-lg">
        ...
        <div className="relative z-10 flex items-center justify-between">
          <div>
            ...
            <p className="text-sm text-white/80 mt-1">Online Course / Organize courses by category • {(categories || []).length} categories</p>
          </div>
          <span className="hidden md:inline-flex items-center gap-1.5 text-white/90 text-xs bg-white/15 backdrop-blur rounded-full px-3 py-1.5 border border-white/20"><Tag className="h-3.5 w-3.5" /> Categories</span>
        </div>
      </div>
        ...
        <div className="flex-1 min-w-0">
          <div className="bg-white rounded-xl border border-gray-200 shadow-sm">
              ...
              <table className="w-full text-sm">
                <thead>
                  <tr className="bg-gray-50 border-b border-gray-200">
                    <th className="text-left px-4 py-3 font-semibold text-gray-600 text-xs uppercase">Category Name</th>
            ...
            <div className="px-4 py-3 border-t border-gray-200 text-sm text-gray-500">
              <span>Showing {categories?.length || 0} records</span>
            </div>
          </div>
        </div>
      </div>
