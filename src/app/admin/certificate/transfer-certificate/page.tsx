...
const conductOptions = ["Good", "Satisfactory", "Poor"]
...
export default function TransferCertificatePage() {
  ...
  const [selectedStudent, setSelectedStudent] = useState("Select")
  const [showModal, setShowModal] = useState(false)
  const [checkedStudents, setCheckedStudents] = useState<number[]>([])
  const { data: tcs, add, remove } = useApi<TransferCertificate>("/api/certificate/student")
  ...
  const filteredStudents = students.filter((s) => {
    ...
    if (selectedStudent !== "Select" && s.name !== selectedStudent) return true
    return true
  })
  ...
  return (
    <div className="space-y-5">
      <div className="relative overflow-hidden rounded-xl bg-gradient-to-r from-[var(--primary)] to-[var(--primary)]/80 px-6 py-4 shadow-sm">
        <h1 className="text-xl font-semibold text-white">Transfer Certificate</h1>
        <p className="mt-1 text-sm text-white/80">Certificate / Transfer Certificate</p>
      </div>
      ...
      <div className="rounded-xl border border-gray-200 bg-white shadow-sm p-5">
        ...
        <div className="grid grid-cols-1 md:grid-cols-4 gap-4">
          ...
          <div>
            ...
            <select value={selectedStudent} onChange={(e) => setSelectedStudent(e.target.value)} className="w-full rounded-lg border border-gray-300 px-3 py-2 text-sm focus:border-transparent focus:ring-2 focus:ring-[var(--primary)]">
      ...
      <div className="rounded-xl border border-gray-200 bg-white shadow-sm">
        <div className="p-5 border-b border-gray-100 flex items-center justify-between">
          ...
          {checkedStudents.length > 0 && (
            <button onClick={handleGenerateTC} className="rounded-lg bg-[var(--primary)] px-4 py-2 text-sm font-medium text-white hover:opacity-90">
              Generate TC
            </button>
          )}
        </div>
      ...
      <div className="rounded-xl border border-gray-200 bg-white shadow-sm">
        <div className="p-5 border-b border-gray-100">
          <h2 className="text-sm font-semibold text-gray-800">Generated Transfer Certificates</h2>
        </div>
        <div className="overflow-x-auto">
          <table className="w-full text-sm">
            <thead>
              <tr className="bg-gray-50 border-b border-gray-200">
                ...
                <th className="text-left px-4 py-3 font-semibold text-gray-600 text-xs">TC No</th>
                <th className="text-left px-4 py-3 font-semibold text-gray-600 text-xs">Issue Date</th>
                <th className="text-left px-4 py-3 font-semibold text-gray-600 text-xs">Action</th>
              </tr>
            </thead>
            <tbody>
              {tcs.map((tc, idx) => (
                <tr key={tc.id} className={`${idx % 2 === 0 ? "bg-white" : "bg-gray-50/50"} hover:bg-gray-50 border-b border-gray-100`}>
                  <td className="px-4 py-3 text-gray-700">{idx + 1}</td>
                  <td className="px-4 py-3 text-gray-700">{tc.studentName}</td>
                  <td className="px-4 py-3 text-gray-700">{tc.tcNo}</td>
                  <td className="px-4 py-3 text-gray-700">{tc.issueDate}</td>
                  <td className="px-4 py-3">
                    <div className="flex items-center gap-2">
                      ...
                      <button onClick={() => handleDelete(tc.id)} className="p-1.5 text-red-600 hover:bg-red-50 rounded-lg" title="Delete"><Trash2 className="h-4 w-4" /></button>
                    </div>
                  </td>
                </tr>
              ))}
        ...
        <div className="fixed inset-0 z-50 flex items-center justify-center bg-black/50">
          <div className="w-full max-w-md rounded-xl bg-white p-6 shadow-lg">
            <div className="flex items-center justify-between mb-4">
              <h3 className="text-base font-semibold text-gray-800">Generate Transfer Certificate</h3>
            ...
            <div className="space-y-4">
              <div>
                ...
                <input type="text" value={dateOfIssue} onChange={(e) => setDateOfIssue(e.target.value)} placeholder="MM/DD/YYYY" className="w-full rounded-lg border border-gray-300 px-3 py-2 text-sm focus:border-transparent focus:ring-2 focus:ring-[var(--primary)]" />
              </div>
              <div>
                <label className="block text-xs font-medium text-gray-600 mb-1">Date of Leaving</label>
                <input type="text" value={dateOfLeaving} onChange={(e) => setDateOfLeaving(e.target.value)} placeholder="MM/DD/YYYY" className="w-full rounded-lg border border-gray-300 px-3 py-2 text-sm focus:border-transparent focus:ring-2 focus:ring-[var(--primary)]" />
              </div>
              <div>
                <label className="block text-xs font-medium text-gray-600 mb-1">Reason</label>
                <textarea value={reason} onChange={(e) => setReason(e.target.value)} rows={3} className="w-full rounded-lg border border-gray-300 px-3 py-2 text-sm focus:border-transparent focus:ring-2 focus:ring-[var(--primary)]" />
              </div>
              <div>
                <label className="block text-xs font-medium text-gray-600 mb-1">Conduct</label>
                <select value={conduct} onChange={(e) => setConduct(e.target.value)} className="w-full rounded-lg border border-gray-300 px-3 py-2 text-sm focus:border-transparent focus:ring-2 focus:ring-[var(--primary)]">
                  {conductOptions.map((opt) => <option key={opt} value={opt}>{opt}</option>)}
                </select>
              </div>
              <div>
                <label className="block text-xs font-medium text-gray-600 mb-1">Remarks</label>
                <textarea value={remarks} onChange={(e) => setRemarks(e.target.value)} rows={3} className="w-full rounded-lg border border-gray-300 px-3 py-2 text-sm focus:border-transparent focus:ring-2 focus:ring-[var(--primary)]" />
              </div>
            </div>
            <div className="flex justify-end gap-3 mt-6">
              ...
              <button onClick={handleSubmitTC} className="rounded-lg bg-[var(--primary)] px-4 py-2 text-sm font-medium text-white hover:opacity-90 flex items-center gap-1.5"><Save className="h-4 w-4" />Save</button>
            </div>
          </div>
        </div>
      )}
