...
export default function VideoTutorialPage() {
  ...
  const handleSaveAdd = async () => {
    ...
    await add({ title: form.title.trim(), videoUrl: form.videoUrl.trim(), className: form.className, subject: form.subject, description: form.description.trim() });
    setForm(emptyForm);
  };

  const handleEdit = (item: VideoTutorial) => {
    setEditing(item);
    setForm({ title: item.title, videoUrl: item.videoUrl, className: item.className, subject: item.subject, description: item.description });
    setShowModal(true);
  };

  const handleUpdate = async () => {
    if (!form.title.trim() || !form.videoUrl.trim()) return;
    await update(editing!.id, { title: form.title.trim(), videoUrl: form.videoUrl.trim(), className: form.className, subject: form.subject, description: form.description.trim() });
    setShowModal(false);
    setEditing(null);
    setForm(emptyForm);
  };
  ...
  return (
    <div className="space-y-6 p-6">
      <div className="relative overflow-hidden rounded-xl bg-gradient-to-r from-[var(--primary)] to-[var(--primary)]/80 px-6 py-4 shadow-sm">
        <h1 className="text-xl font-semibold text-white">Video Tutorials</h1>
        <p className="mt-1 text-sm text-white/80">Manage educational video tutorials</p>
      </div>

      <div className="grid grid-cols-1 gap-6 lg:grid-cols-2">
        <div className="rounded-xl border border-gray-200 bg-white p-6 shadow-sm">
          <h2 className="mb-4 text-base font-semibold text-[var(--title-color)]">Add Video Tutorial</h2>
          <div className="space-y-4">
            ...
            <div>
              <label className="mb-1 block text-sm font-medium text-[var(--foreground)]">Video URL</label>
              <input
                type="text"
                value={form.videoUrl}
        ...
        <div className="overflow-hidden rounded-xl border border-gray-200 bg-white shadow-sm">
          <table className="w-full text-sm">
            <thead>
              <tr className="border-b border-gray-200 bg-gray-50">
                ...
                <th className="px-4 py-3 text-left font-medium text-[var(--title-color)]">Description</th>
            ...
            <tbody>
              {videos.map((video, idx) => (
                ...
                >
                  ...
                  <td className="px-4 py-3 font-medium text-[var(--title-color)]">{video.title}</td>
                  <td className="px-4 py-3 text-[var(--foreground)]">{video.className}</td>
                  <td className="px-4 py-3 text-[var(--foreground)]">{video.subject}</td>
                  <td className="px-4 py-3 text-[var(--foreground)]">{truncate(video.description, 40)}</td>
          ...
          {videos.length === 0 && (
            <div className="px-4 py-8 text-center text-sm text-[var(--subtitle-color)]">No video tutorials found</div>
          )}
        </div>
      </div>
        ...
        <div className="fixed inset-0 z-50 flex items-center justify-center bg-black/50">
          <div className="w-full max-w-md rounded-xl bg-white p-6 shadow-lg">
            <div className="mb-4 flex items-center justify-between">
              <h2 className="text-lg font-semibold text-[var(--title-color)]">Edit Video Tutorial</h2>
