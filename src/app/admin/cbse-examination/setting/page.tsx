...
export default function CbseSettingPage() {
  const { data: terms, add: addTerm, update: updateTerm, remove: removeTerm } = useApi<Term>("/api/cbse/terms")
  const { data: obsParams, add: addObsParam, update: updateObsParam, remove: removeObsParam } = useApi<ObsParam>("/api/cbse/obs-params")
  const { data: assessments, add: addAssessment, update: updateAssessment, remove: removeAssessment } = useApi<Assessment>("/api/cbse/assessments")
  const { data: examGrades, add: addExamGrade, update: updateExamGrade, remove: removeExamGrade } = useApi<ExamGrade>("/api/cbse/exam-grades")
    ...
    <div className="space-y-6">
      <div className="relative overflow-hidden rounded-xl bg-gradient-to-r from-[var(--primary)] to-[var(--primary)]/80 px-6 py-4 shadow-sm">
        <div className="relative z-10">
          ...
          <p className="text-sm text-[var(--primary)]/80 mt-0.5">CBSE Examination / Setting</p>
        </div>
      </div>
