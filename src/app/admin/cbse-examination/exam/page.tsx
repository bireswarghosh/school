...
export default function CBSEExamPage() {
  const { data: exams, add, update, remove, loading } = useApi<Exam>("/api/cbse/exam")
  ...
  const { data: apiTerms } = useApi<ApiTerm>("/api/cbse/terms")
  const { data: apiAssessments } = useApi<ApiAssessment>("/api/cbse/assessments")
  const { data: apiGrades } = useApi<ApiGrade>("/api/cbse/exam-grades")
  const { data: apiTemplates } = useApi<Template>("/api/cbse/template")
  ...
  const openStudentsModal = async (exam: Exam) => {
    ...
    try {
      const res = await fetch(`/api/cbse/exam-students?exam_id=${exam.id}`)
      if (res.ok) {
  ...
  const saveStudents = async () => {
    ...
    try {
      await fetch(`/api/cbse/exam-students?exam_id=${selectedExam.id}`, { method: "DELETE" })
      for (const studentId of selectedStudents) {
        await fetch("/api/cbse/exam-students", {
          method: "POST",
  ...
  const openSubjectsModal = async (exam: Exam) => {
    ...
    try {
      const res = await fetch(`/api/cbse/exam-subjects?exam_id=${exam.id}`)
      if (res.ok) {
        const data = await res.json()
        setSubjects(data)
      }
  ...
  const addOrEditSubject = async () => {
    ...
    try {
      if (editSubjectId !== null) {
        const res = await fetch("/api/cbse/exam-subjects", {
          method: "PUT",
          headers: { "Content-Type": "application/json" },
  ...
  const deleteSubject = async (id: number) => {
    try {
      await fetch(`/api/cbse/exam-subjects?id=${id}`, { method: "DELETE" })
  ...
  const loadMarks = async (examId: number, subjectId: number) => {
    try {
      const res = await fetch(`/api/cbse/exam-marks?exam_id=${examId}&subject_id=${subjectId}`)
      if (res.ok) {
        const data = await res.json()
  ...
  const saveMarks = async () => {
    ...
    if (!subject) return
    try {
      await fetch(`/api/cbse/exam-marks?exam_id=${selectedExam.id}&subject_id=${subject.id}`, { method: "DELETE" })
      for (const marks of Object.values(studentMarks)) {
        for (const mark of marks) {
          await fetch("/api/cbse/exam-marks", {
            method: "POST",
            headers: { "Content-Type": "application/json" },
            body: JSON.stringify({
  ...
  const openAttendanceModal = async (exam: Exam) => {
    ...
    try {
      const res = await fetch(`/api/cbse/exam-attendance?exam_id=${exam.id}`)
      if (res.ok) {
        const data = await res.json()
  ...
  const saveAttendance = async () => {
    if (!selectedExam) return
    try {
      await fetch(`/api/cbse/exam-attendance?exam_id=${selectedExam.id}`, { method: "DELETE" })
      for (const [studentId, records] of Object.entries(studentAttendance)) {
        for (const rec of records) {
          await fetch("/api/cbse/exam-attendance", {
            method: "POST",
            headers: { "Content-Type": "application/json" },
            body: JSON.stringify({
              exam_id: selectedExam.id,
  ...
  const openRemarkModal = async (exam: Exam) => {
    ...
    try {
      const [attRes, marksRes] = await Promise.all([
        fetch(`/api/cbse/exam-attendance?exam_id=${exam.id}`),
        fetch(`/api/cbse/exam-marks?exam_id=${exam.id}`),
      ])
  ...
  const saveRemarks = async () => {
    if (!selectedExam) return
    try {
      const existing = await (await fetch(`/api/cbse/exam-attendance?exam_id=${selectedExam.id}`)).json()
    ...
    <div className="space-y-6">
      <div className="relative overflow-hidden rounded-xl bg-gradient-to-r from-[var(--primary)] to-[var(--primary)]/80 px-6 py-4 shadow-sm">
        <div className="relative z-10">
          <h2 className="text-xl font-bold text-white">CBSE Exam</h2>
          <p className="text-sm text-[var(--primary)]/80 mt-0.5">CBSE Examination / Exam</p>
        </div>
      </div>
