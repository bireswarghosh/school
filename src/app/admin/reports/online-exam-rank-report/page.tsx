...
const scoreOf = (r: any) =>
  Number(r?.score ?? r?.obtained_marks ?? r?.obtainedMarks ?? r?.marks ?? r?.total_marks ?? 0)
...
export default function OnlineExamRankReportPage() {
  ...
  const ranked = useMemo(
    () => [...attempts].sort((a, b) => scoreOf(b) - scoreOf(a)),
    [attempts]
  )
  ...
  return (
    <div className="space-y-6">
      <ReportBanner title="Exams Rank Report" subtitle="Reports / Online Examinations / Exams Rank Report" />
      ...
      <TableCard title={`Exams Rank Report${selectedExam ? ` · ${selectedExam.name}` : ""} (${ranked.length})`} action={<PrintButtons />}>
          ...
          <table className="w-full text-sm">
            ...
            <tbody className="divide-y divide-gray-100">
              {loading ? (
                ...
                <EmptyRow colSpan={8} message="Please select an exam to view ranks" />
              ) : ranked.length === 0 ? (
                <EmptyRow colSpan={8} message="No attempts found" />
              ) : (
                ranked.map((r, i) => (
        ...
        {!loading && examId && <FooterCount shown={ranked.length} total={attempts.length} />}
      </TableCard>
    </div>
  )
}
