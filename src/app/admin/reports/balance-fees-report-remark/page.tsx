...
export default function BalanceFeesReportRemarkPage() {
  ...
  const grouped = useMemo(() => {
    ...
    for (const r of rows) {
      ...
      if (!g.remark) g.remark = r.remarks || r.note || ""
      m.set(key, g)
    }
  ...
  return (
    <div className="space-y-6">
      <ReportBanner title="Balance Fees Report With Remark" subtitle="Reports / Finance / Balance Fees Report With Remark" />
      ...
      <TableCard title={`Balance Fees Report With Remark (${grouped.length})`} action={<PrintButtons />}>
                ...
                grouped.map((g, i) => {
                  ...
                  return (
                    <tr key={`${g.studentId}-${g.feesType}`} className="hover:bg-orange-50/50">
                      ...
                      <td className="px-4 py-2.5">{g.remark || "—"}</td>
                    </tr>
                  )
                })
              )}
