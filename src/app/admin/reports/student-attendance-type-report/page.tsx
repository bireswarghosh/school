...
export default function StudentAttendanceTypeReportPage() {
  ...
  const sortedTypes = useMemo(() => [...attTypes].sort((a, b) => a.id - b.id), [attTypes])
  ...
  const filtered = useMemo(() => {
    return rows.filter((r) => {
      ...
      if (typeId && String(r.attendanceTypeId) !== typeId) return false
      return true
    })
  }, [rows, classId, sectionId, typeId])

  return (
    <div className="space-y-6">
      <ReportBanner title="Student Attendance Type Report" subtitle="Reports / Attendance / Student Attendance Type Report" />
      ...
      <TableCard title={`Student Attendance Type Report (${filtered.length})`} action={<PrintButtons />}>
