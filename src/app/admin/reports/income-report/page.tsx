...
export default function IncomeReportPage() {
  ...
  const heads = useMemo(() => {
    const all = income.map((r) => headName(r.incomeHead)).filter((h) => h !== "—")
  ...
  const rows = useMemo(() => {
    return income.filter((r) => {
      ...
      if (head && headName(r.incomeHead) !== head) return false
      return true
    })
  }, [income, from, to, head])
  ...
  return (
    <div className="space-y-6">
      <ReportBanner title="Income Report" subtitle="Reports / Finance / Income Report" />
      ...
      <TableCard title={`Income Report (${rows.length})`} action={<PrintButtons />}>
        ...
        {!loading && <FooterCount shown={rows.length} total={income.length} />}
      </TableCard>
    </div>
  )
}
