...
export default function ICSCExamSchedulePage() {
  const { data: exams } = useApi<Exam>("/api/icsc/exam")
  ...
  const fetchSchedules = useCallback(async (examId: string) => {
    setLoading(true)
    try {
      const url = examId ? `/api/icsc/schedule?exam_id=${examId}` : "/api/icsc/schedule"
  ...
  const handleAdd = async () => {
    ...
    try {
      const res = await fetch("/api/icsc/schedule", {
        method: "POST",
  ...
  const confirmDelete = async () => {
    ...
    try {
      await fetch(`/api/icsc/schedule?id=${deleteId}`, { method: "DELETE" })
    ...
    <div className="space-y-6">
      <div className="relative overflow-hidden rounded-xl bg-gradient-to-r from-[var(--primary)] to-[var(--primary)]/80 px-6 py-4 shadow-sm">
        <div className="relative z-10">
          ...
          <p className="text-sm text-white/80 mt-0.5">ICSC Examination / Exam Timetable</p>
        </div>
      </div>
