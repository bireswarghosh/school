...
export default function IcscAdmitCardPage() {
  const { classNames: classes, sectionNames } = useClassesAndSections();
  const sectionOptions = ["Select All", ...sectionNames];
  const { data: admitCards, loading } = useApi<AdmitCardRecord>("/api/icsc/admit-card")
    ...
    <div className="space-y-6">
      <div className="relative overflow-hidden rounded-xl bg-gradient-to-r from-[var(--primary)] to-[var(--primary)]/80 px-6 py-4 shadow-sm">
        <div className="relative z-10">
          ...
          <p className="text-sm text-[var(--primary)]/80 mt-0.5">ICSC Examination / Admit Card</p>
        </div>
      </div>
