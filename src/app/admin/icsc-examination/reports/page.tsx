...
export default function IcscReportsPage() {
  const { classNames } = useClassesAndSections()
  const classes = ["All", ...classNames]
  const { data: subjectReports } = useApi<SubjectReport>("/api/icsc/reports?type=subject-marks")
  const { data: templateReports } = useApi<TemplateReport>("/api/icsc/reports?type=template-marks")
    ...
    <div className="space-y-6">
      <div className="relative overflow-hidden rounded-xl bg-gradient-to-r from-[var(--primary)] to-[var(--primary)]/80 px-6 py-4 shadow-sm">
        <div className="relative z-10">
          ...
          <p className="text-sm text-[var(--primary)]/80 mt-0.5">ICSC Examination / Reports</p>
        </div>
      </div>
