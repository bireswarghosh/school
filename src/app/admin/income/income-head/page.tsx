...
export default function IncomeHeadPage() {
  const { data: heads, add, update, remove } = useApi<IncomeHead>("/api/income/head")
  ...
  const handleAdd = async () => {
    ...
    if (!name.trim()) errs.name = "Income head is required"
    else if (heads?.some((h) => h.name.toLowerCase() === name.trim().toLowerCase())) errs.name = "Income head already exists"
    setErrors(errs)
  ...
  const handleEditOpen = (head: IncomeHead) => {
    setEditId(head.id)
  ...
  const handleEditSave = async () => {
    const errs: Record<string, string> = {}
    if (!editName.trim()) errs.name = "Income head is required"
    else if (heads?.some((h) => h.name.toLowerCase() === editName.trim().toLowerCase() && h.id !== editId)) errs.name = "Income head already exists"
    setEditErrors(errs)
    if (Object.keys(errs).length) return
    ...
    <div className="space-y-6">
      <div className="relative overflow-hidden rounded-xl bg-gradient-to-r from-[var(--primary)] to-[var(--primary)]/80 px-6 py-4 shadow-sm">
        <div className="relative z-10">
          <h2 className="text-xl font-bold text-white">Income Head</h2>
          <p className="text-sm text-[var(--primary)]/80 mt-0.5">Income / Income Head</p>
        </div>
      </div>
      ...
      <div className="flex flex-col lg:flex-row gap-6">
        <div className="w-full lg:w-80 flex-shrink-0">
          <div className="bg-white rounded-xl border border-gray-200 shadow-sm">
            <div className="px-5 py-3 border-b border-gray-200">
              <h3 className="text-sm font-semibold text-gray-700">Add Income Head</h3>
            </div>
            <div className="p-5 space-y-4">
              <div>
                <label className="block text-sm font-medium text-gray-700 mb-1">Income Head <span className="text-red-500">*</span></label>
                <input type="text" value={name} onChange={(e) => { setName(e.target.value); if (errors.name) setErrors({}) }}
                  placeholder="Enter income head" className="w-full px-3 py-2 border border-gray-300 rounded-lg text-sm focus:ring-2 focus:ring-[var(--primary)] focus:border-transparent" />
        ...
        <div className="flex-1 min-w-0">
          <div className="bg-white rounded-xl border border-gray-200 shadow-sm">
            <div className="px-5 py-3 border-b border-gray-200">
              <h3 className="text-sm font-semibold text-gray-700">Income Head List</h3>
            </div>
              ...
              <table className="w-full text-sm">
                <thead>
                  <tr className="bg-gray-50 border-b border-gray-200">
                    ...
                    <th className="text-left px-4 py-3 font-semibold text-gray-600 text-xs uppercase">Income Head</th>
                ...
                <tbody>
                  {(heads || []).length === 0 ? (
                    <tr><td colSpan={4} className="text-center py-8 text-gray-400">No income heads found</td></tr>
                  ) : (
                    (heads || []).map((head, idx) => (
        ...
        <div className="fixed inset-0 z-50 flex items-center justify-center p-4">
          ...
          <div className="relative bg-white rounded-xl shadow-2xl w-full max-w-md z-10">
            <div className="flex items-center justify-between px-5 py-4 border-b border-gray-200">
              <h3 className="text-base font-semibold text-gray-800">Edit Income Head</h3>
      ...
      {showDeleteModal && (
        <div className="fixed inset-0 z-50 flex items-center justify-center p-4">
          ...
          <div className="relative bg-white rounded-xl shadow-2xl w-full max-w-md z-10">
            ...
            <div className="p-5">
              <p className="text-sm text-gray-600">Are you sure you want to delete this income head?
