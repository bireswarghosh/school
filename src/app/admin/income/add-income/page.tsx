...
const paymentModes = ["Cash", "Cheque", "Online Transfer", "Card", "DD", "Other"]
...
export default function AddIncomePage() {
  ...
  const { data: incomes, add, update, remove } = useApi<IncomeRecord>("/api/income")
  ...
  const [form, setForm] = useState({ incomeHeadId: "", name: "", invoiceNo: "", date: "", amount: "", description: "", paymentMode: "", note: "", document: "" })
  const [editForm, setEditForm] = useState({ id: 0, incomeHeadId: "", name: "", invoiceNo: "", date: "", amount: "", description: "", paymentMode: "", note: "", document: "" })
  const [errors, setErrors] = useState<Record<string, string>>({})

  const filtered = (incomes || []).filter((inc) =>
    inc.name.toLowerCase().includes(search.toLowerCase()) ||
    inc.invoiceNo.toLowerCase().includes(search.toLowerCase()) ||
    (inc.incomeHead || "").toLowerCase().includes(search.toLowerCase())
  )

  const handleFormChange = (e: React.ChangeEvent<HTMLInputElement | HTMLTextAreaElement | HTMLSelectElement>, isEdit = false) => {
    const { name, value } = e.target
    const setter = isEdit
      ? (fn: any) => setEditForm((prev) => ({ ...prev, [name]: value }))
      : (fn: any) => setForm((prev) => ({ ...prev, [name]: value }))
    setter(null)
  ...
  const validateForm = (data: typeof form) => {
    ...
    if (!data.incomeHeadId) errs.incomeHeadId = "Income head is required"
  ...
  const handleAdd = async () => {
    ...
    await add({
      incomeHeadId: parseInt(form.incomeHeadId), name: form.name.trim(), invoiceNo: form.invoiceNo,
    ...
    setForm({ incomeHeadId: "", name: "", invoiceNo: "", date: "", amount: "", description: "", paymentMode: "", note: "", document: "" })
    setShowAddModal(false)
  }

  const handleEditOpen = (inc: IncomeRecord) => {
    setEditForm({
      id: inc.id, incomeHeadId: inc.incomeHeadId?.toString() || "", name: inc.name,
      invoiceNo: inc.invoiceNo || "", date: inc.date, amount: inc.amount.toString(),
      description: inc.description || "", paymentMode: inc.paymentMode || "", note: inc.note || "", document: inc.document || "",
    })
    setErrors({})
  ...
  const handleEditSave = async () => {
    ...
    await update(editForm.id, {
      incomeHeadId: parseInt(editForm.incomeHeadId), name: editForm.name.trim(), invoiceNo: editForm.invoiceNo,
  ...
  const FormFields = ({ data, onChange, isEdit }: { data: typeof form; onChange: (e: any) => void; isEdit?: boolean }) => (
    <div className="px-6 py-4 space-y-4">
      <div className="grid grid-cols-2 gap-4">
        <div className="space-y-1">
          <label className="block text-xs font-medium text-gray-600">Income Head <span className="text-red-500">*</span></label>
          <select name="incomeHeadId" value={data.incomeHeadId} onChange={onChange}
          ...
          {errors.incomeHeadId && <p className="text-red-500 text-xs mt-1">{errors.incomeHeadId}</p>}
        </div>
        <div className="space-y-1">
      ...
      <div className="grid grid-cols-2 gap-4">
        ...
        <div className="space-y-1">
          ...
          <input type="text" name="note" value={data.note} onChange={onChange} placeholder="Optional note"
            className="w-full px-3 py-2 text-sm border border-gray-300 rounded-lg focus:ring-2 focus:ring-[var(--primary)] focus:border-transparent" />
        </div>
      </div>
      <div className="space-y-1">
      ...
      <div className="space-y-1">
        ...
        <input type="file" onChange={(e) => {
          ...
          const handler = isEdit ? setEditForm : setForm
          handler((prev: any) => ({ ...prev, document: file ? file.name : "" }))
        }}
    ...
    <div className="space-y-6">
      <div className="relative overflow-hidden rounded-xl bg-gradient-to-r from-[var(--primary)] to-[var(--primary)]/80 px-6 py-4 shadow-sm">
        <div className="relative z-10">
          ...
          <p className="text-sm text-[var(--primary)]/80 mt-0.5">Income / Add Income</p>
        </div>
      </div>

      <div className="bg-white rounded-xl border border-gray-200 shadow-sm">
        <div className="px-5 py-3 border-b border-gray-200 flex flex-wrap items-center justify-between gap-4">
          <h3 className="text-sm font-semibold text-gray-700">Income List</h3>
          <div className="flex items-center gap-3">
            ...
            <button onClick={() => { setForm({ incomeHeadId: "", name: "", invoiceNo: "", date: "", amount: "", description: "", paymentMode: "", note: "", document: "" }); setErrors({}); setShowAddModal(true) }}
          ...
          <table className="w-full text-sm">
            <thead>
              <tr className="bg-gray-50 border-b border-gray-200">
                {["#", "Name", "Income Head", "Invoice No", "Date", `Amount (${symbol})`, "Payment Mode", "Description", "Document", "Action"].map((h) => (
            ...
            <tbody>
              {filtered.length === 0 ? (
                <tr><td colSpan={10} className="text-center py-8 text-gray-400">No income records found</td></tr>
              ) : (
                filtered.map((inc, idx) => (
                  <tr key={inc.id} className={`border-b border-gray-100 hover:bg-gray-50 transition-colors ${idx % 2 === 1 ? "bg-gray-50/50" : ""}`}>
                    ...
                    <td className="px-4 py-3 text-gray-600 max-w-[180px] truncate">{inc.description || "—"}</td>
                    <td className="px-4 py-3 text-gray-600 max-w-[120px] truncate">{inc.document || "—"}</td>
                    <td className="px-4 py-3">
                      <div className="flex items-center gap-1">
                        <button onClick={() => handleEditOpen(inc)} className="p-1.5 text-amber-600 hover:bg-amber-50 rounded-lg transition-colors" title="Edit"><Pencil className="h-4 w-4" /></button>
                        <button onClick={() => handleDeleteOpen(inc.id)} className="p-1.5 text-red-600 hover:bg-red-50 rounded-lg transition-colors" title="Delete"><Trash2 className="h-4 w-4" /></button>
                      </div>
                    </td>
                  </tr>
                ))
        ...
        <div className="px-4 py-3 border-t border-gray-200 text-sm text-gray-500">
          <span>Showing {filtered.length} of {incomes?.length || 0} records</span>
        </div>
      </div>

      <Modal title="Add Income" show={showAddModal} onClose={() => setShowAddModal(false)} wide>
        {FormFields({ data: form, onChange: handleAddFormChange })}
      ...
      <Modal title="Edit Income" show={showEditModal} onClose={() => setShowEditModal(false)} wide>
      ...
      {showDeleteModal && (
        <div className="fixed inset-0 z-50 flex items-center justify-center p-4">
          ...
          <div className="relative bg-white rounded-xl shadow-2xl w-full max-w-md z-10">
            ...
            <div className="p-5">
              <p className="text-sm text-gray-600">Are you sure you want to delete this income record?
                {deleteId && <strong className="block mt-1 text-gray-800">{(incomes || []).find((i) => i.id === deleteId)?.name}</strong>}
              </p>
            </div>
