...
import { Pencil, Trash2, X, Save, Download, Upload, Printer, ChevronLeft, ChevronRight, Search } from "lucide-react"
import { useApi } from "@/lib/use-api"

type FeesType = { id: number; name: string; feesCode: string; description: string }

const today = () => new Date().toISOString().split("T")[0]

export default function FeesTypePage() {
  const { data: types, add, update, remove } = useApi<FeesType>("/api/fees/fees-type")
  ...
  const [editFeesCode, setEditFeesCode] = useState("")
  ...
  const handleAdd = async () => {
    ...
    if (!feesCode.trim()) errs.feesCode = "Fees code is required"
    setErrors(errs)
    if (Object.keys(errs).length) return
    await add({ name: name.trim(), feesCode: feesCode.trim(), description: description.trim() })
    setName(""); setFeesCode(""); setDescription("")
  }

  const openEdit = (type: FeesType) => {
    setEditId(type.id); setEditName(type.name); setEditFeesCode(type.feesCode || ""); setEditDescription(type.description || ""); setEditErrors({}); setShowEditModal(true)
  }

  const handleEditSave = async () => {
    const errs: Record<string, string> = {}
    if (!editName.trim()) errs.name = "Name is required"
    if (!editFeesCode.trim()) errs.feesCode = "Fees code is required"
    setEditErrors(errs)
    if (Object.keys(errs).length) return
    await update(editId!, { name: editName.trim(), feesCode: editFeesCode.trim(), description: editDescription.trim() })
  ...
  const filtered = useMemo(() => {
    ...
    return currentData.filter((t) => t.name.toLowerCase().includes(kw) || (t.feesCode || "").toLowerCase().includes(kw) || (t.description || "").toLowerCase().includes(kw))
  }, [currentData, keyword])
  ...
  const exportCSV = () => {
    const rows = currentData.map((t) => `"${t.name}","${(t.feesCode || "").replace(/"/g, '""')}","${(t.description || "").replace(/"/g, '""')}"`)
    const blob = new Blob(["\uFEFFName,Fees Code,Description\n" + rows.join("\n")], { type: "text/csv;charset=utf-8" })
    const a = document.createElement("a"); a.href = URL.createObjectURL(blob); a.download = "Fees_Type.csv"; a.click()
  }

  const exportExcel = () => {
    const rows = currentData.map((t) => `<tr><td>${t.name}</td><td>${t.feesCode || ""}</td><td>${t.description || ""}</td></tr>`).join("")
    const blob = new Blob([`<html><meta charset="utf-8"><body><table><tr><th>Name</th><th>Fees Code</th><th>Description</th></tr>${rows}</table></body></html>`], { type: "application/vnd.ms-excel" })
    const a = document.createElement("a"); a.href = URL.createObjectURL(blob); a.download = "Fees_Type.xls"; a.click()
  }
  ...
  const printTable = () => {
    ...
    if (win) {
      win.document.write(`<html><head><title>Fees Type</title><style>body{font-family:Arial;font-size:12px;padding:20px}table{width:100%;border-collapse:collapse}th,td{border:1px solid #ccc;padding:6px 10px;text-align:left}th{background:#f5f5f5}</style></head><body><h2>Fees Type</h2><table><tr><th>Name</th><th>Fees Code</th><th>Description</th></tr>${rows}</table><p style="color:#999;font-size:11px;margin-top:10px">Generated on ${today()}</p></body></html>`)
  ...
  const importCSV = async (e: React.ChangeEvent<HTMLInputElement>) => {
    ...
    for (let i = 1; i < lines.length; i++) {
      ...
      if (cols[0]) parsed.push({ name: cols[0], feesCode: cols[1] || null, description: cols[2] || null })
    }
    if (parsed.length === 0) { notify.error("No valid records"); return }
    try {
      const res = await fetch("/api/fees/fees-type", { method: "POST", headers: { "Content-Type": "application/json" }, body: JSON.stringify(parsed) })
    ...
    <div className="space-y-5">
      <div className="relative overflow-hidden rounded-xl bg-gradient-to-r from-[var(--primary)] to-[var(--primary)]/80 px-6 py-4 shadow-sm">
        <div className="relative z-10">
          <h2 className="text-lg font-bold text-white">Fees Type</h2>
          <p className="text-xs text-white/80 mt-0.5">Fees Collection / Fees Type</p>
        </div>
      </div>
      ...
      <div className="flex flex-col lg:flex-row gap-5">
        <div className="w-full lg:w-80 flex-shrink-0">
          <div className="bg-white rounded-xl border border-gray-200 shadow-sm">
            <div className="px-5 py-3 border-b border-gray-100"><h3 className="text-sm font-semibold text-gray-800">Add Fees Type</h3></div>
            <div className="p-5 space-y-4">
              <div><label className="block text-xs font-medium text-gray-600 mb-1">Name <span className="text-red-400">*</span></label>
                <input type="text" value={name} onChange={(e) => { setName(e.target.value); if (errors.name) setErrors({}) }} placeholder="Enter fees type name" className="w-full h-9 px-3 text-sm border border-gray-300 rounded-lg focus:ring-2 focus:ring-[var(--primary)]" />
                {errors.name && <p className="text-red-400 text-xs mt-0.5">{errors.name}</p>}</div>
              <div><label className="block text-xs font-medium text-gray-600 mb-1">Fees Code <span className="text-red-400">*</span></label>
                <input type="text" value={feesCode} onChange={(e) => { setFeesCode(e.target.value); if (errors.feesCode) setErrors({}) }} placeholder="Enter fees code" className="w-full h-9 px-3 text-sm border border-gray-300 rounded-lg focus:ring-2 focus:ring-[var(--primary)]" />
                {errors.feesCode && <p className="text-red-400 text-xs mt-0.5">{errors.feesCode}</p>}</div>
        ...
        <div className="flex-1 min-w-0">
          <div className="bg-white rounded-xl border border-gray-200 shadow-sm overflow-hidden">
            <div className="px-5 py-3 border-b border-gray-100"><h3 className="text-sm font-semibold text-gray-800">Fees Type List</h3></div>
            <div className="overflow-x-auto">
              <table className="w-full text-sm">
                <thead>
                  <tr className="bg-gradient-to-r from-gray-50 to-gray-100/50 border-b border-gray-200">
                    <th className="text-left px-4 py-2.5 font-semibold text-gray-600 text-[11px] uppercase">Name</th>
                    <th className="text-left px-4 py-2.5 font-semibold text-gray-600 text-[11px] uppercase">Fees Code</th>
                    <th className="text-right px-4 py-2.5 font-semibold text-gray-600 text-[11px] uppercase">Action</th>
                  </tr>
                </thead>
                <tbody>
                  {paginated.length === 0 ? (
                    <tr><td colSpan={3} className="text-center py-12 text-gray-400 text-sm">No fees types found</td></tr>
                  ) : (
                    paginated.map((type, idx) => (
                      <tr key={type.id} className={`border-b border-gray-50 hover:bg-[var(--primary-light)] transition-colors ${idx % 2 === 0 ? "bg-white" : "bg-gray-50/30"}`}>
                        <td className="px-4 py-2.5 font-medium text-gray-800">{type.name}</td>
                        <td className="px-4 py-2.5 text-gray-600 font-mono text-xs">{type.feesCode}</td>
                        <td className="px-4 py-2.5 text-right">
                          <div className="flex items-center justify-end gap-0.5">
                            <button onClick={() => openEdit(type)} className="p-1.5 text-amber-600 hover:bg-amber-50 rounded-lg" title="Edit"><Pencil className="h-3.5 w-3.5" /></button>
                            <button onClick={() => openDelete(type.id)} className="p-1.5 text-red-600 hover:bg-red-50 rounded-lg" title="Delete"><Trash2 className="h-3.5 w-3.5" /></button>
                          </div>
                        </td>
                      </tr>
                    ))
        ...
        <div className="fixed inset-0 z-50 flex items-center justify-center p-4">
          ...
          <div className="relative bg-white rounded-xl shadow-2xl w-full max-w-lg z-10">
            <div className="flex items-center justify-between px-5 py-3 border-b border-gray-200">
              <h3 className="text-base font-semibold text-gray-800">Edit Fees Type</h3>
            ...
            <div className="p-5 space-y-4">
              <div><label className="block text-xs font-medium text-gray-600 mb-1">Name <span className="text-red-400">*</span></label>
                <input type="text" value={editName} onChange={(e) => { setEditName(e.target.value); if (editErrors.name) setEditErrors({}) }} placeholder="Enter fees type name" className="w-full h-9 px-3 text-sm border border-gray-300 rounded-lg focus:ring-2 focus:ring-[var(--primary)]" />
                {editErrors.name && <p className="text-red-400 text-xs mt-0.5">{editErrors.name}</p>}</div>
              <div><label className="block text-xs font-medium text-gray-600 mb-1">Fees Code <span className="text-red-400">*</span></label>
                <input type="text" value={editFeesCode} onChange={(e) => { setEditFeesCode(e.target.value); if (editErrors.feesCode) setEditErrors({}) }} placeholder="Enter fees code" className="w-full h-9 px-3 text-sm border border-gray-300 rounded-lg focus:ring-2 focus:ring-[var(--primary)]" />
                {editErrors.feesCode && <p className="text-red-400 text-xs mt-0.5">{editErrors.feesCode}</p>}</div>
              <div><label className="block text-xs font-medium text-gray-600 mb-1">Description</label>
        ...
        <div className="fixed inset-0 z-50 flex items-center justify-center p-4">
          ...
          <div className="relative bg-white rounded-xl shadow-2xl w-full max-w-lg z-10">
            ...
            <div className="p-5 text-center">
              ...
              <p className="text-sm text-gray-600">Are you sure you want to delete this fees type?</p>
              {deleteId && <p className="text-sm font-semibold text-gray-800 mt-1">{(types || []).find((t) => t.id === deleteId)?.name}</p>}
            </div>
            <div className="px-5 py-3 border-t border-gray-200 flex justify-end gap-2">
