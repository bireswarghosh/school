...
const reportData: ZoomClassReport[] = [
  { id: 1, title: "Mathematics - Algebra Basics", date: "07/03/2026", duration: "45", staff: "John Doe", class: "Class 1", section: "A", status: "Awaited", totalJoined: 28, joinedStudents: [
    { name: "Alice Johnson", admissionNo: "ADM001" }, { name: "Bob Smith", admissionNo: "ADM002" }, { name: "Charlie Brown", admissionNo: "ADM003" },
  ]},
  { id: 2, title: "Science - Plant Biology", date: "07/03/2026", duration: "40", staff: "Jane Smith", class: "Class 2", section: "B", status: "Started", totalJoined: 22, joinedStudents: [
    { name: "Diana Prince", admissionNo: "ADM004" }, { name: "Edward Norton", admissionNo: "ADM005" },
  ]},
  { id: 3, title: "English Grammar Session", date: "07/02/2026", duration: "35", staff: "Robert Wilson", class: "Class 3", section: "A", status: "Finished", totalJoined: 30, joinedStudents: [
    { name: "Fiona Apple", admissionNo: "ADM006" }, { name: "George Lucas", admissionNo: "ADM007" }, { name: "Hannah Montana", admissionNo: "ADM008" },
  ]},
  { id: 4, title: "History - Ancient Civilizations", date: "07/04/2026", duration: "50", staff: "Emily Davis", class: "Class 4", section: "C", status: "Cancelled", totalJoined: 0, joinedStudents: []},
]
...
export default function ZoomLiveClassesReportPage() {
  const { classNames: classes, sectionNames: sections } = useClassesAndSections()
  const [filter, setFilter] = useState({ class: "", section: "" })
  const [filteredData, setFilteredData] = useState<ZoomClassReport[] | null>(null)
  const [viewJoinList, setViewJoinList] = useState<ZoomClassReport | null>(null)
  ...
  return (
    <div className="space-y-6">
      <div>
        ...
        <p className="text-sm text-gray-500 mt-1">Zoom Live Classes / Live Classes Report</p>
      </div>
          ...
          <table className="w-full text-sm">
            <thead>
              <tr className="border-b border-gray-200 bg-gray-50">
                {["#", "Class Title", "Date", "Duration", "Staff", "Class-Section", "Status", "Total Joined", "Action"].map((h) => (
            ...
            <tbody>
              {(filteredData || reportData).map((r, idx) => (
                <tr key={r.id} className={`border-b border-gray-100 ${idx % 2 === 0 ? "bg-white" : "bg-gray-50/50"} hover:bg-[var(--primary-light)]`}>
                  ...
                  <td className="px-4 py-2.5 text-gray-600">{r.class}-{r.section}</td>
