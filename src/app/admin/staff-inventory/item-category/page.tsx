...
import { Search, Plus, Pencil, Trash2, X, Layers, RotateCcw } from "lucide-react"
...
export default function ItemCategoryPage() {
  const { data: categories, add, update, remove } = useApi<Category>("/api/staff-inventory/item-category")
    ...
    <div className="space-y-6">
      <div className="relative overflow-hidden rounded-xl bg-gradient-to-r from-[var(--primary)] to-[var(--primary)]/80 px-6 py-4 shadow-sm">
        <div className="relative z-10">
          <h2 className="text-xl font-bold text-white">Item Category</h2>
          <p className="text-sm text-white/80 mt-1">Staff Inventory / Item Category</p>
        </div>
      </div>

      <div className="rounded-xl border border-gray-200 bg-white shadow-sm">
        <div className="px-5 py-3 border-b border-gray-200 flex items-center justify-between">
          <h3 className="text-sm font-semibold text-gray-700">Item Category List</h3>
        ...
        <div className="px-4 py-3 border-t border-gray-200 flex items-center justify-between text-sm text-gray-500">
          <span>Showing {filtered.length} of {categories.length} records</span>
        </div>
      </div>
          ...
          <div className="relative bg-white rounded-xl shadow-2xl w-full max-w-md max-h-[90vh] overflow-y-auto z-10">
            ...
            <div className="p-6 space-y-4">
              <div>
                ...
                <input type="text" value={form.name} onChange={(e) => { setForm({ ...form, name: e.target.value }); if (formErrors.name) setFormErrors({}) }} className="w-full rounded-lg border border-gray-300 px-3 py-2 text-sm focus:border-transparent focus:ring-2 focus:ring-[var(--primary)]" placeholder="Enter category name" />
        ...
        <div className="fixed inset-0 z-50 flex items-center justify-center p-4">
          ...
          <div className="relative bg-white rounded-xl shadow-2xl w-full max-w-md z-10">
            ...
            <div className="p-6">
              ...
              {deleteId && <p className="text-sm font-semibold text-gray-800 mt-1">{categories.find((c) => c.id === deleteId)?.name}</p>}
            </div>
            <div className="px-6 py-4 border-t border-gray-200 flex justify-end gap-2">
