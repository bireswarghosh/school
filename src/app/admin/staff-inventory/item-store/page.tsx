...
const initialForm = { name: "", code: "", location: "", inchargeName: "", description: "" }

export default function ItemStorePage() {
  const { data: stores, add, update, remove } = useApi<Store>("/api/staff-inventory/store")
  ...
  const filtered = useMemo(() => {
    return stores.filter((s) => {
      if (filter && !`${s.name} ${s.code} ${s.location}`.toLowerCase().includes(filter.toLowerCase())) return false
      return true
    })
  }, [stores, filter])
  ...
  const handleAdd = async () => {
    ...
    await add({
      name: form.name.trim(), code: form.code.trim(), location: form.location,
      inchargeName: form.inchargeName, description: form.description,
    })
    setForm(initialForm)
    ...
    <div className="space-y-6">
      <div className="relative overflow-hidden rounded-xl bg-gradient-to-r from-[var(--primary)] to-[var(--primary)]/80 px-6 py-4 shadow-sm">
        <div className="relative z-10">
          <h2 className="text-xl font-bold text-white">Item Store</h2>
          <p className="text-sm text-white/80 mt-1">Staff Inventory / Item Store</p>
        </div>
      </div>

      <div className="rounded-xl border border-gray-200 bg-white shadow-sm">
        <div className="px-5 py-3 border-b border-gray-200 flex items-center justify-between">
          <h3 className="text-sm font-semibold text-gray-700">Item Store List</h3>
              ...
              {filtered.length === 0 ? (
                ...
                filtered.map((s, idx) => (
                  <tr key={s.id} className={`border-b border-gray-100 hover:bg-gray-50 transition-colors ${idx % 2 === 1 ? "bg-gray-50/50" : ""}`}>
                    ...
                    <td className="px-4 py-3 text-gray-600">{s.inchargeName || "-"}</td>
            ...
            <div className="p-6 space-y-4">
              <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                ...
                <div>
                  ...
                  <input type="text" value={form.code} onChange={(e) => { setForm({ ...form, code: e.target.value }); if (formErrors.code) setFormErrors({}) }} className="w-full rounded-lg border border-gray-300 px-3 py-2 text-sm focus:border-transparent focus:ring-2 focus:ring-[var(--primary)]" placeholder="Enter store code" />
              ...
              <div>
                ...
                <input type="text" value={form.inchargeName} onChange={(e) => setForm({ ...form, inchargeName: e.target.value })} className="w-full rounded-lg border border-gray-300 px-3 py-2 text-sm focus:border-transparent focus:ring-2 focus:ring-[var(--primary)]" placeholder="Enter incharge name" />
              </div>
              <div>
        ...
        <div className="fixed inset-0 z-50 flex items-center justify-center p-4">
          ...
          <div className="relative bg-white rounded-xl shadow-2xl w-full max-w-lg max-h-[90vh] overflow-y-auto z-10">
            ...
            <div className="p-6 space-y-4">
              ...
              <div>
                <label className="block text-sm font-medium text-gray-700 mb-1">Incharge Name</label>
                <input type="text" value={editForm.inchargeName ?? ""} onChange={(e) => setEditForm({ ...editForm, inchargeName: e.target.value })} className="w-full rounded-lg border border-gray-300 px-3 py-2 text-sm focus:border-transparent focus:ring-2 focus:ring-[var(--primary)]" />
              </div>
              <div>
                <label className="block text-sm font-medium text-gray-700 mb-1">Description</label>
